...
if [ ! -f ${VERTICADATA}/config/admintools.conf ]; then
  ...
  su - dbadmin -c "/opt/vertica/bin/admintools -t create_db --skip-fs-checks -d ${DATABASE_NAME} ${DBPW} -c ${VERTICADATA}/catalog -D ${VERTICADATA}/data --hosts localhost --timeout=never --policy=always " 
else
